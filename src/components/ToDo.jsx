...
export function ToDo(props) {
    ...
    function countTask(){
        if(!check){
            props.onSetCountTask(props.countTask + 1)
        }else{
            props.onSetCountTask(props.countTask - 1)
        }
    }
    return (
        <div className={styles.taskContainer}>
            <div onClick={() => {setCheck(!check), countTask()}} htmlFor='checkbox' className={check ? styles.checkBoxChecked : styles.checkBox}>
                <svg className={check ? styles.check : styles.notCheck} xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="var(--gray-100)" viewBox="0 0 10 7">
                    <path d="M8.43059 0.342093L4.09865 4.67403L1.61618 2.19156L0.780273 3.02747L4.09865 6.34584L9.26649 1.178L8.43059 0.342093Z" fill="#F2F2F2" />
                </svg>
            </div>
            <span className={check ? styles.textChecked : styles.text}>{props.content}</span>
            <div onClick={() => {
                    props.onDeleteTask(props.content)
                    if(check) props.onSetCountTask(props.countTask)                
                }} className={styles.trash}>
                <svg width="20" height="20" viewBox="0 0 13 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
